 ...
 * Init the ajax 
 */
function jobs_ajax_init() {
	elgg_register_page_handler('search_endpoint', 'jobs_ajax_endpoint_handler');
	elgg_extend_view('page/elements/head', 'metatags/jquery_autocomplete', 1000);


	elgg_register_plugin_hook_handler('search', 'countries_short', 'jobs_search_countries_short');

	elgg_register_plugin_hook_handler('search_types', 'get_types', 'jobs_search_register_country_types');
}

function jobs_ajax_endpoint_handler($page = array()) {
 ...
 *  
 */
function jobs_search_get_registered_entity_types() {
 ...
 *  return array
 */
function jobs_search_get_filters_list() {
	$filter_list = array();

	$types = jobs_search_get_registered_entity_types();
...
function jobs_search_extract_src_from_img_tag($img_tag) {
	$values = array();
	$src = '';
...
function jobs_search_countries_short($hook, $object_type, $returnvalue, $params) {
	$search_type = FALSE;
	...
	$all_countries = KtJobBaseMain::ktform_get_country_values();
...
function jobs_search_register_country_types($hook, $object_type, $returnvalue, $params) {
	$returnvalue['countries_short'] = 'countries_short';
	return $returnvalue;
}

elgg_register_event_handler('init', 'system', 'jobs_ajax_init');