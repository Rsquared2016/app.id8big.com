...
function jobs_images_init() {
	global $CONFIG;

	elgg_extend_view('css','images/css');

	elgg_register_page_handler('jobs_icon','jobs_images_page_handler');
	
	elgg_register_event_handler('create', 'object', 'jobs_images_image_hander');
	elgg_register_event_handler('update', 'object', 'jobs_images_image_hander');
	
	elgg_register_event_handler('create', 'group', 'jobs_images_image_hander');
	elgg_register_event_handler('update', 'group', 'jobs_images_image_hander');
	
	
	// Now override icons
	elgg_register_plugin_hook_handler('entity:icon:url', 'all', 'jobs_images_icon_hook', 999);
}


function jobs_images_page_handler($page) {
	global $CONFIG;
	...
	include $CONFIG->pluginspath . 'jobs/ktform/pages/images/thumb.php';

	return TRUE;
}

function jobs_images_image_hander($event, $object_type, $object) {
	...
	if($check_event && $check_objects && $object->getType() == 'object' && $object->getSubType() == 'job') {
		...
		if(isset($_SESSION['images']['input_image'])) {
			...
			if (isset($_FILES[$input_image]) && $_FILES[$input_image]['error'] == 0) {
				...
				$prefix = jobs_images_get_prefix($object);
				...
				if(isset($image_options['resize']) && $image_options['resize']) {
					...
					$icon_sizes = elgg_trigger_plugin_hook('jobs_images:icon_sizes', 'object', array('entity' => $object), $icon_sizes);
					...
					foreach ($icon_sizes as $name => $size_info) {
						...
						$resized = jobs_images_get_resized_image_from_existing_file($_FILES[$input_image]['tmp_name'], $size_info['w'], $size_info['h'], $size_info['square'], 0, 0, 0, 0, $size_info['upscale']);
...
function jobs_images_icon_hook($hook, $entity_type, $returnvalue, $params){
	global $CONFIG;

	if ((!$returnvalue) && ($hook == 'entity:icon:url') && ($params['entity'] instanceof KtJob)) {
		...
		if($file_exists) {
			$url = $CONFIG->url . "jobs_icon/$prefix/$entity_guid/$size/$icontime.jpg";
			return $url;
		}
	}
}

function jobs_images_get_entity_icon_url($entity, $size, $viewtype = '') {
	...
	return jobs_images_icon_hook($hook, $entity->getType(), '', $params);
}


function jobs_images_get_prefix($object) {
	$otype = $object->type;
 ...
 */
function jobs_images_get_resized_image_from_existing_file($input_name, $maxwidth, $maxheight, $square = FALSE, $x1 = 0, $y1 = 0, $x2 = 0, $y2 = 0, $upscale = FALSE) {
...
elgg_register_event_handler('init', 'system', 'jobs_images_init');