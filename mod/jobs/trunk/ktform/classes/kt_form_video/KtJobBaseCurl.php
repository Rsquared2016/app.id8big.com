...
/*
 * This is a class from izap videos, with some modifications to be used into jobs_ktform.
 * Thank you Izap Guys. :)
 */
  ...
  function &asPostString(&$theData, $theName = NULL) {
    ...
    if (is_array($theData)) {
      foreach ($theData as $theKey => $theValue) {
        if ($thePrefix === NULL) {
          $thePostString .= '&' . KtJobBaseCurl::asPostString($theValue, $theKey) ;
        }
        else {
          $thePostString .= '&' . KtJobBaseCurl::asPostString($theValue, $thePrefix . '[' . $theKey . ']') ;
        }
      }
    }
    else {
...
class jobs_xml2array {
  function jobs_xml2array( $xml ) {
    if ( file_exists($xml) )
