...
if ($vars['options_values']) {
	foreach($vars['options_values'] as $value => $option) {
		...
		if (is_array($option)) {
			...
			foreach($option as $optgroup_value =>$optgroup_option) {
				$encoded_value = htmlentities($optgroup_value, ENT_QUOTES, 'UTF-8');
				$encoded_option = htmlentities($optgroup_option, ENT_QUOTES, 'UTF-8');

				if ((string)$optgroup_value == (string)$vars['value']) {
