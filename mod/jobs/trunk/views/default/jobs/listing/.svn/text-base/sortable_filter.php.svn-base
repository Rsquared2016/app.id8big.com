...
$extra_fields = KtJobBaseMain::ktform_get_extra_listing_fields($object_subtype);
...
$extra_field_key_name = 'jobs_ktform:sorteable:link';
?>

<div class="sortingWrapper">
	<h5 class="sortingTitle"><?php echo elgg_echo('jobs:sort:title') ?></h5>
	...
	<?php
		foreach ($sortable_fields as $internalname => $options) {
			...
			$key_text = KtJobBaseMain::ktform_generate_sortable_link($internalname, $key_text);
			$li_class = 'sortableItem sortableHeader';

			$sort_type = KtJobBaseMain::ktform_camelize_string($sort_type, TRUE);
