...
/**
 * This library pretend to save files to one object handled with jobs_ktform input file
 */
//Constant that defines the file name, that will handle the session and the filefactory
define('KtJob_st_FILE_NAME', 'kt_file');

function jobs_file_init() {
	/**
	 * Register the events
	 */
//	elgg_register_event_handler('create', 'user', 'jobs_file_hander', 999);
	elgg_register_event_handler('profileupdate', 'user', 'jobs_file_hander', 999);
	
	elgg_register_event_handler('create', 'object', 'jobs_file_hander', 999);
	elgg_register_event_handler('update', 'object', 'jobs_file_hander', 999);
	
	elgg_register_event_handler('update', 'group', 'jobs_file_hander', 999);
	elgg_register_event_handler('update', 'group', 'jobs_file_hander', 999);

	elgg_register_page_handler('kt_file', 'jobs_files_handler');

	elgg_register_action('jobs/download/file', dirname(dirname(__FILE__)) . '/actions/kt_file/download_a.php', TRUE);
}

function jobs_files_handler($page) {
	switch ($page[0]) {
		default:
			if (isset($page[0])) {
...
function jobs_file_hander($event, $object_type, $object) {
	...
	if ($check_event && $check_object) {
		...
		if(!array_key_exists(KtJob_st_FILE_NAME, $_SESSION)) {
			return TRUE;
		}
		 ...
		 * and object subtype
		 */
		$KT_FILE = $_SESSION[KtJob_st_FILE_NAME];
		if (!is_array($KT_FILE)) {
			return TRUE;
		}
		...
		unset($_SESSION[KtJob_st_FILE_NAME]);

		$owner_guid = $object->getOwnerGUID();
		$object_guid = $object->getGUID();

		$prefix = jobs_file_get_prefix($object);
		...
		foreach ($file_inputs as $internalname => $input) {
			if (isset($_FILES[$internalname]['name']) && !empty($_FILES[$internalname]['name'])) {
				...
				$existent_filename = jobs_get_filestore_data($object, $internalname, 'filename');
				...
				$filestore_save_data = array(
					...
					'simpletype' => jobs_get_general_file_type($_FILES[$internalname]['type']),
					'size' => $file->size(),
				);
...
function jobs_file_get_prefix($object) {
	...
	$prefix = $prefix . DIRECTORY_SEPARATOR . KtJob_st_FILE_NAME;

	return $prefix;
}

function jobs_get_filestore_data($object, $internalname, $return_key = FALSE) {
 ...
 */
function jobs_get_general_file_type($mimetype) {
...
elgg_register_event_handler('init', 'system', 'jobs_file_init');