 ...
 */
define('KtJob_st_INLINE_GROUP_STRING', 'inline');
...
class KtJobBaseForm {
	...
	private static $BUNDLE_NAME = 'jobs';
...
//		$this->setSaver('KtJobBaseSave');

		$this->setValidator('KtJobBaseValidators');
	 ...
	 * 	by default it will return KtJobBaseSave string, if none is setted.
	 * 
	 * @return string 
	 */
	 ...
	 * 	by default will return KtJobBaseValidators string, if none is setted
	 * 
	 * @return string 
	 */
	 ...
	 * @param string $class the KtJobBaseForm class name
	 * 
	 * @return Instance KtJobBaseSave | FALSE if no success 
	 */
	...
	public function setTypes($type = 'object', $subtype = '') {
		...
		switch ($this->type) {
			case 'group':
				$this->setSaver('KtJobBaseGroupSave');
				break;
			default:
				$this->setSaver('KtJobBaseSave');
				break;
	...
	public function setObject($object_value) {
		...
		if (!$this->__isNewEntity()) {
			if (($object->getSubtype() != $this->getSubType()) || $this->getType() != $object->getType()) {
				throw new KtJobBaseInputException(elgg_echo('jobs:error:no_equal_subtype_and_type'));
			}
		}
	...
	public function setInput($input_options = array()) {
		...
		if (!(array_key_exists('internalname', $input_options) && !empty($input_options['internalname']))) {
			throw new KtJobBaseInputException(elgg_echo('jobs:error:empty:internalname'));
		}

		if (!(array_key_exists('type', $input_options) && !empty($input_options['type']))) {
			throw new KtJobBaseInputException(elgg_echo('jobs:error:empty:type'));
		}
		...
		if (!empty($input_options['label'])) {
			if (is_bool($input_options['label'])) {
				$input_options['label'] = elgg_echo("jobs:{$this->getPluginName()}:label:{$input_options['internalname']}");
			} else {
		...
		if ($input_options['info_text']) {
			if (is_bool($input_options['info_text'])) {
				$input_options['info_text'] = elgg_echo("jobs:{$this->getPluginName()}:info_text:{$input_options['internalname']}");
			} else {
	...
	public function setFields($form_fields = array()) {
		...
		$form_fields = elgg_trigger_plugin_hook('jobs:fields', $plugin_hook_name, NULL, $form_fields);
	...
	public function setFormVars($form_vars = array()) {
		...
		$defaults = array(
			 ...
			 'js' => 'onsubmit="javascript:return jobs_ktform_process_submit(this);"',
			 'internalid' => '',
			 'internalname' => '',
			 'enctype' => '',
		...
		$custom_rel = KtJob_st_AJAX_REL_ATTR;
	...
	/**
	 * Set the css classname of the container div inside the view input/jobs_submit.
	 * 
	...
	public function renderFieldsToArray($render_options = array()) {
		...
		$submit_text_elgg = "jobs:{$plugin_name}:label:submit:send";
		...
		foreach ($form_fields as $field) {
			...
			if ($field['type'] == 'text') {
				if (is_array($field['options'])) {
					if ($field['internalname'] == 'title') {
						$js_max_length_input = 'maxlength=' . KtJob_st_MAX_LENGHT_TITLE;
					} else {
						$js_max_length_input = 'maxlength=' . KtJob_st_MAX_LENGHT_TEXT;
					}
		...
		foreach ($form_fields as $internalname => $field) {
			...
			if ($object_value != '') {
				...
				switch ($type) {
					...
					case 'dropdown':
					case 'pulldown':
						$options['value'] = KtJobBaseMain::get_values_for_outputs_with_options($options, FALSE);
						break;
					case 'checkboxes':
						$options['value'] = KtJobBaseMain::get_values_for_outputs_with_options($options);
						break;
...
//						break;
	...
	public function render($render_options = array(), $before_footer_string = '') {
		...
		$body_form_class = KtJobBaseMain::ktform_camelize_string($this->getPluginName(), FALSE);
		...
		$extraClass = '';
		if (elgg_get_plugin_setting('profile_label_above', 'jobs') == 'yes') {
		...
		foreach ($rendered_group_fields as $rendered_group => $rendered_fields) {
			...
			$is_inline = (bool) strstr($rendered_group, KtJob_st_INLINE_GROUP_STRING);
			$title = '';
	...
	public function save($register_errors = TRUE, $override_data = array()) {
		...
		if ($success != FALSE && ($type || $subtype)) {
			...
			$guid = $this->getObject();
			//$saver = new KtJobBaseSave($type, $subtype, $class);
...
//			}
	...
	private function __setSessionField($internalname, $value) {
		...
		if ($value != '') {
			$_SESSION['jobs'][$this->getPluginName()] = $this->session;
		}
	...
	private function __getSessionValue($internalname) {
		$session = $_SESSION['jobs'][$this->getPluginName()];
	...
	private function __getSessions() {
		return $_SESSION['jobs'][$this->getPluginName()];
	}
	...
	public function __clearSession() {
		...
		$_SESSION['jobs'][$this->getPluginName()] = NULL;
		unset($_SESSION['jobs'][$this->getPluginName()]);
	}
