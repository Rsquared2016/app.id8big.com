...
?>
<?php
//if (defined('Help_st_DISABLE_BULK_ACTIONS') && Help_st_DISABLE_BULK_ACTIONS != TRUE) {
if($kt_chck) {
	//Get the defined bulk actions for this tab/content.
	$bulk_actions = HelpBaseMain::ktform_get_bulk_action_links($entity_subtype);
...
<div class="ktFrmBulkActionsWrapper">
	<form method="post" action="#" class="ktToolsTop">
		...
		<label class="ktChkLabel">
			...
			<span class="txt"><?php echo elgg_echo('help_ktform:bulk_actions:label:all'); ?></span>
		</label>
		<?php
		//Bulk actions.
		...
		?>
		<input type="submit" class="btnSnd" value="<?php echo elgg_echo('help_ktform:bulk_actions:label:apply'); ?>" />
		<div class="cThis">&nbsp;</div>
	</form>
</div>
<?php } ?>