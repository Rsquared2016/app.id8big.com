...
//<script type="text/javascript">
	...
	function help_ktform_process_submit(element) {
		...
		var savingTextDefault = '<?php echo elgg_echo('help_ktform:form:add:submit:default:text') ?>';
	...
	function() {
		...
		$('.ktFrmBulkActionsWrapper form').submit(function(){
			...
			if(actionSel == 'undefined' || actionSel == '') {
				alert('<?php echo elgg_echo('help_ktform:bulk_actions:error:select:an:action') ?>');
				return false;
			} else {
				//Add action.
			...
			if($('.chckCont input[type=checkbox].ktChk:checked').length == 0) {
				alert('<?php echo elgg_echo('help_ktform:bulk_actions:error:check:a:checkbox') ?>')
				return false;
			} else {
				//Add guids.
		...
		$('form[rel=<?php echo json_encode(Help_st_AJAX_REL_ATTR) ?>]').live('submit', function(event) {
