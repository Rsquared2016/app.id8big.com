...
$extra_fields = HelpBaseMain::ktform_get_extra_listing_fields($object_subtype);
...
$extra_field_key_name = 'help_ktform:sorteable:link';
?>

<div class="sortingWrapper">
	<h5 class="sortingTitle"><?php echo elgg_echo('help:sort:title') ?></h5>
	...
	<?php
		foreach ($sortable_fields as $internalname => $options) {
			...
			$key_text = HelpBaseMain::ktform_generate_sortable_link($internalname, $key_text);
			$li_class = 'sortableItem sortableHeader';

			$sort_type = HelpBaseMain::ktform_camelize_string($sort_type, TRUE);
