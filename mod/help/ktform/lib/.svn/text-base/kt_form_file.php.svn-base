...
/**
 * This library pretend to save files to one object handled with help_ktform input file
 */
//Constant that defines the file name, that will handle the session and the filefactory
define('Help_st_FILE_NAME', 'kt_file');

function help_file_init() {
	/**
	 * Register the events
	 */
//	elgg_register_event_handler('create', 'user', 'help_file_hander', 999);
	elgg_register_event_handler('profileupdate', 'user', 'help_file_hander', 999);
	
	elgg_register_event_handler('create', 'object', 'help_file_hander', 999);
	elgg_register_event_handler('update', 'object', 'help_file_hander', 999);
	
	elgg_register_event_handler('update', 'group', 'help_file_hander', 999);
	elgg_register_event_handler('update', 'group', 'help_file_hander', 999);

	elgg_register_page_handler('kt_file', 'help_files_handler');

	elgg_register_action('help/download/file', dirname(dirname(__FILE__)) . '/actions/kt_file/download_a.php', TRUE);
}

function help_files_handler($page) {
	switch ($page[0]) {
		default:
			if (isset($page[0])) {
...
function help_file_hander($event, $object_type, $object) {
	...
	if ($check_event && $check_object) {
		...
		if(!array_key_exists(Help_st_FILE_NAME, $_SESSION)) {
			return TRUE;
		}
		 ...
		 * and object subtype
		 */
		$KT_FILE = $_SESSION[Help_st_FILE_NAME];
		if (!is_array($KT_FILE)) {
			return TRUE;
		}
		...
		unset($_SESSION[Help_st_FILE_NAME]);

		$owner_guid = $object->getOwnerGUID();
		$object_guid = $object->getGUID();

		$prefix = help_file_get_prefix($object);
		...
		foreach ($file_inputs as $internalname => $input) {
			if (isset($_FILES[$internalname]['name']) && !empty($_FILES[$internalname]['name'])) {
				...
				$existent_filename = help_get_filestore_data($object, $internalname, 'filename');
				...
				$filestore_save_data = array(
					...
					'simpletype' => help_get_general_file_type($_FILES[$internalname]['type']),
					'size' => $file->size(),
				);
...
function help_file_get_prefix($object) {
	...
	$prefix = $prefix . DIRECTORY_SEPARATOR . Help_st_FILE_NAME;

	return $prefix;
}

function help_get_filestore_data($object, $internalname, $return_key = FALSE) {
 ...
 */
function help_get_general_file_type($mimetype) {
...
elgg_register_event_handler('init', 'system', 'help_file_init');