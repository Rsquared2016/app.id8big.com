...
/**
 * Description of HelpBaseGroupSave
 *
 * @author Bortoli German <gbortoli@keetup.com>
 */
class HelpBaseGroupSave extends HelpBaseSave {
	...
	public function save($guid = 0, $data = array(), $entity_options = array()) {
		...
		$default_entity_options = array(
			...
			'container_guid' => HelpBaseMain::ktform_get_container_guid(),
//			'access_id' => get_default_access(), //ACCESS_PUBLIC, //ACCESS_DEFAULT,
		...
		$error_object_key_name = 'help_ktform:save';
		...
		if ($data['name']) {
			if (defined('Help_st_MAX_LENGHT_TEXT')) {
				$entity->name = elgg_get_excerpt($data['name'], Help_st_MAX_LENGHT_TEXT);
			} else {
