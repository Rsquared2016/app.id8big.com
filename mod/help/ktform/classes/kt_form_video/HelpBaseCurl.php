...
/*
 * This is a class from izap videos, with some modifications to be used into help_ktform.
 * Thank you Izap Guys. :)
 */
  ...
  function &asPostString(&$theData, $theName = NULL) {
    ...
    if (is_array($theData)) {
      foreach ($theData as $theKey => $theValue) {
        if ($thePrefix === NULL) {
          $thePostString .= '&' . HelpBaseCurl::asPostString($theValue, $theKey) ;
        }
        else {
          $thePostString .= '&' . HelpBaseCurl::asPostString($theValue, $thePrefix . '[' . $theKey . ']') ;
        }
      }
    }
    else {
...
class help_xml2array {
  function help_xml2array( $xml ) {
    if ( file_exists($xml) )
