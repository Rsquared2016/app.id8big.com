...
/**
 * GDriveBaseForm profile display
 *
...
if ($vars['full_view']) {
	...
	switch($display_type) {
		case 'full':
				echo elgg_view('gdrive/profile_full', $vars);
			break;
		default:
				echo elgg_view('gdrive/profile', $vars);
			break;
	}
	...
	switch ($option_viewtype) {
		...
		case 'full':
			echo elgg_view('gdrive/listing_full', $vars);
		break;

		case 'gallery':
			echo elgg_view('gdrive/gallery', $vars);
		break;

		case 'widget':
			echo elgg_view('gdrive/listing_widget', $vars);
		break;

		default:
			echo elgg_view('gdrive/listing', $vars);
		break;
	}
}
