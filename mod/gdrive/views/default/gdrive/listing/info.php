...
//gdrive_ktform_get_trimed_title().
$title = $entity->title;
if (!$title) {
	$title = $entity->name;
}
...
if(strlen($title) > GDrive_st_MAX_LENGHT_TITLE_LISTING){
	$title = elgg_get_excerpt($title, GDrive_st_MAX_LENGHT_TITLE_LISTING);
}
?>
...
?>
	<h4>
		...
		<?php
			...
			echo elgg_view('gdrive/listing/info_middle_extend', $vars);
...
<?php
	 ...
	 * */
	$extra_fields = GDriveBaseMain::ktform_get_extra_listing_fields($entity_subtype);
 ...
 * */
//echo elgg_view('gdrive/tools/action_wheel', $vars);
