...
/**
 * This library pretend to save files to one object handled with gdrive_ktform input file
 */
//Constant that defines the file name, that will handle the session and the filefactory
define('GDrive_st_FILE_NAME', 'gdrive_kt_file');

function gdrive_file_init() {
	/**
	 * Register the events
	 */
//	elgg_register_event_handler('create', 'user', 'gdrive_file_hander', 999);
	elgg_register_event_handler('profileupdate', 'user', 'gdrive_file_hander', 999);
	
	elgg_register_event_handler('create', 'object', 'gdrive_file_hander', 999);
	elgg_register_event_handler('update', 'object', 'gdrive_file_hander', 999);
	
	elgg_register_event_handler('update', 'group', 'gdrive_file_hander', 999);
	elgg_register_event_handler('update', 'group', 'gdrive_file_hander', 999);

	elgg_register_page_handler('gdrive_kt_file', 'gdrive_files_handler');

	elgg_register_action('gdrive/download/file', dirname(dirname(__FILE__)) . '/actions/kt_file/download_a.php', TRUE);
}

function gdrive_files_handler($page) {
	switch ($page[0]) {
		default:
			if (isset($page[0])) {
...
function gdrive_file_hander($event, $object_type, $object) {
	...
	if ($check_event && $check_object) {
		...
		if(!array_key_exists(GDrive_st_FILE_NAME, $_SESSION)) {
			return TRUE;
		}
		 ...
		 * and object subtype
		 */
		$KT_FILE = $_SESSION[GDrive_st_FILE_NAME];
		if (!is_array($KT_FILE)) {
			return TRUE;
		}
		...
		unset($_SESSION[GDrive_st_FILE_NAME]);

		$owner_guid = $object->getOwnerGUID();
		$object_guid = $object->getGUID();

		$prefix = gdrive_file_get_prefix($object);
		...
		foreach ($file_inputs as $internalname => $input) {
			if (isset($_FILES[$internalname]['name']) && !empty($_FILES[$internalname]['name'])) {
				...
				$existent_filename = gdrive_get_filestore_data($object, $internalname, 'filename');
				...
				$filestore_save_data = array(
					...
					'simpletype' => gdrive_get_general_file_type($_FILES[$internalname]['type']),
					'size' => $file->size(),
				);
...
function gdrive_file_get_prefix($object) {
	...
	$prefix = $prefix . DIRECTORY_SEPARATOR . GDrive_st_FILE_NAME;

	return $prefix;
}

function gdrive_get_filestore_data($object, $internalname, $return_key = FALSE) {
 ...
 */
function gdrive_get_general_file_type($mimetype) {
...
elgg_register_event_handler('init', 'system', 'gdrive_file_init');