  ...
  function &asPostString(&$theData, $theName = NULL) {
    ...
    if (is_array($theData)) {
      foreach ($theData as $theKey => $theValue) {
        if ($thePrefix === NULL) {
          $thePostString .= '&' . GDriveBaseCurl::asPostString($theValue, $theKey) ;
        }
        else {
          $thePostString .= '&' . GDriveBaseCurl::asPostString($theValue, $thePrefix . '[' . $theKey . ']') ;
        }
      }
    }
    else {
...
class gdrive_xml2array {
  function gdrive_xml2array( $xml ) {
    if ( file_exists($xml) )
