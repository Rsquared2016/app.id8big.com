...
/**
 * Description of GDriveBaseGroupSave
 *
 * @author Bortoli German <gbortoli@keetup.com>
 */
class GDriveBaseGroupSave extends GDriveBaseSave {
	...
	public function save($guid = 0, $data = array(), $entity_options = array()) {
		...
		$default_entity_options = array(
			...
			'container_guid' => GDriveBaseMain::ktform_get_container_guid(),
//			'access_id' => get_default_access(), //ACCESS_PUBLIC, //ACCESS_DEFAULT,
		...
		$error_object_key_name = 'gdrive_ktform:save';
		...
		if ($data['name']) {
			if (defined('GDrive_st_MAX_LENGHT_TEXT')) {
				$entity->name = elgg_get_excerpt($data['name'], GDrive_st_MAX_LENGHT_TEXT);
			} else {
