...
// Make sure we actually have permission to edit
	...
	if($entity) {
		...
		// Success message
			system_message(sprintf(elgg_echo("gdrive_ktform:delete:entity:deleted"), elgg_echo('item:object:'.$subtype)));

		} else {
			register_error(sprintf(elgg_echo("gdrive_ktform:delete:entity:not_deleted"), elgg_echo('item:object:'.$subtype)));
		}
	} else {
		register_error(sprintf(elgg_echo("gdrive_ktform:delete:entity:not_deleted"), elgg_echo('item:object:'.$subtype)));
	}
    ...
    if (elgg_instanceof($container, 'group')) {
        $fw = 'gdrive/group/'.$container->getGUID().'/all';
    }
	// Forward
	forward($fw);