...
try {
	foreach ($guids as $guid) {
		...
		if($entity && $entity->canEdit()) {
			...
			throw new DataFormatException(elgg_echo("gdrive_ktform:comments:bulk:action:entity:not:allowed"));
		}
	}
} catch(Exception $e) {
...
if($success) {
	// Success message
	system_message(elgg_echo("gdrive_ktform:comments:bulk:action:entity:comments:$comment_enabled"));
}
