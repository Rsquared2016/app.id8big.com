...
$extra_fields = GtaskBaseMain::ktform_get_extra_listing_full_fields($object_subtype);

//Image support.
$image_support = GtaskBaseMain::ktform_get_entity_image_support($object_subtype);

$icon = FALSE;
if ($image_support) {
//Get icon
	$icon_entity = $vars['entity'];
	if  ($image_support === GtaskBaseMain::$Gtask_st_OWNER_ICON_NAME) {
	...
	<div class="ktFullTxt">
		...
		<div class="ktFullSubTitle">
			...
			<span class="categories">
				...
				echo elgg_view('gtask/listing_full/info_middle_extend', $vars);
				?>
			</span>
		</div>
		...
		<?php if ($extra_fields && is_array($extra_fields) && count($extra_fields)) { ?>
			<ul class='ktFormFields'>
				...
				foreach ($extra_fields as $internalname => $options) {
					...
					$field_label = elgg_echo('gtask_ktform:listing:full:titles:' . $internalname);

					$output_view = $options['output_view'];
					$output_vars = array('value' => $entity->$internalname, 'entity' => $entity);

					$block_div = '';
					$tmp_class = GtaskBaseMain::ktform_camelize_string($internalname, FALSE);
					...
					switch($options['output_view']) {
						case 'output/longtext':
							...
							if (defined('Gtask_st_LISTING_FULL_EXCERPT_LONGTEXT')) {
								$excerpt = Gtask_st_LISTING_FULL_EXCERPT_LONGTEXT;
							} else {
								$excerpt = 250;
							}
		...
		<?php } ?>
		<div class="btnKtFormFull"><?php echo elgg_view('gtask/listing_full/buttons', $vars); ?></div>
	</div>
