...
$entity_subtype = GtaskBaseMain::ktform_get_subtype($vars);
...
//$kt_chck = defined('Gtask_st_DISABLE_BULK_ACTIONS') && Gtask_st_DISABLE_BULK_ACTIONS != TRUE; // yes, there is one!
$kt_chck = GtaskBaseMain::ktform_get_bulk_action_support($entity_subtype);
if ($kt_chck && $entity) {
	$class_chck = 'chckCont';
	if (!$icon) {
...
$owner_link = elgg_view('output/url', array(
	'href' => "gtask/owner/$owner->username",
	'text' => $owner->name,
	'is_trusted' => true,
));
...
//}
$responsive_user = get_user($entity->responsive);
$subtitle = '';
if ($responsive_user) {
    $subtitle = $responsive_user->name;
}

$subtitle = $subtitle.elgg_view('gtask/listing/subtitle_extended', $vars);
...
$tags = $tags.elgg_view('gtask/listing/tags_extended', $vars);
...
$list_body = elgg_view('object/gtask/summary', $params);
