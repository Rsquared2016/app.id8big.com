...
$links = GtaskBaseMain::ktform_get_listing_tabs($plugin_url, $context);
...
GtaskBaseMain::ktform_set_current_tab($links['current_tab']);
...
<ul class="tabGeneric">
	<?php foreach($links as $key_link => $link) {
		...
		$tab_name = elgg_echo('gtask_ktform:custom_tabs:'.$key_link);
		...
		if($key_link == 'mine' && elgg_get_page_owner_guid() != elgg_get_logged_in_user_guid()) {
			...
			$tab_name = sprintf(elgg_echo("gtask_ktform:custom_tabs:{$key_link}:custom"), $elgg_get_page_owner_entity->name);
		} 
