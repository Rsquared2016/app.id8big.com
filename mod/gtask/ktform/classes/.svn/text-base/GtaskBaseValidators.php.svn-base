...
class GtaskBaseValidators {
			 	...
			 	if (is_callable(array($this, $function))) {
					...
					if(!$result['status']) {
						if ($validation == 'required' || $validation == 'file' || $validation == 'date_range' || $validation == 'period_range' || $validation == 'non_empty_array') {
							...
							$error_msg = elgg_echo('gtask_ktform:validation:required:not:valid:value');
							$error = sprintf($error_msg,
								$field_label,
								$result['message']
							);
						} else {
							$error_msg = elgg_echo('gtask_ktform:validation:not:valid:value');
						...
						//throw new GtaskBaseValidationException($error);
						
						$this->setError($field_internalname, $error);
					}
				} else {
					$error = sprintf(elgg_echo('gtask_ktform:validation:undefined:validation'),
						$validation,
						$field_label
					);
					//throw new GtaskBaseValidationException($error);
	...
	public function validateRequired($value, $required = FALSE, $field_label = '') {
		...
		if($required && empty($value)) {
			$result['status'] = false;
			$result['message'] = elgg_echo('gtask_ktform:validation:required:text');
		}
	...
	public function validateMin_length($value, $length = 0, $field_label = '') {
		...
		if($value && $length && strlen($value) < $length) {
			$result['status'] = false;
			$result['message'] = sprintf(elgg_echo('gtask_ktform:validation:min_length:text'), $length);
		}
	...
	public function validateMax_length($value, $length = 0, $field_label = '') {
		...
		if($value && $length && strlen($value) > $length) {
			$result['status'] = false;
			$result['message'] = sprintf(elgg_echo('gtask_ktform:validation:max_length:text'), $length);
		}
	...
	public function validateValue_type($value, $type = '', $field_label = '') {
		...
		if($value && $type) {
			...
			switch($type) {
				...
				default:
					$error = sprintf(elgg_echo('gtask_ktform:validation:value_type:text'), $type, $field_label);
					throw new GtaskBaseValidationException($error);					 
			...
			if(!$sucess) {
				$result['status'] = false;
				$result['message'] = sprintf(elgg_echo('gtask_ktform:validation:value_type:text'), $type);
			}
		}
	...
	public function validateValue_filter($value, $option = array(), $field_label = '') {
		...
		if($value && is_array($option)) {
			...
			if($option['exp']) {
				...
				if (!$status) {
					$message = sprintf(elgg_echo('gtask_ktform:validation:value_filter:text'), $option['exp']);
				}
				...
				if($option['operator'] == 'in') {
					...
					if(in_array($value, $option['values']) === FALSE) {
						$status = false;
						$message = sprintf(elgg_echo('gtask_ktform:validation:value_filter:text:in'), $evaluate);
					}
					
				} else {
					$evaluate = "$value {$option['operator']} {$option['values']}"; 
					if($status = !eval($evaluate)) {
						$message = sprintf(elgg_echo('gtask_ktform:validation:value_filter:text:operand'),
									$option['operator'],
									$option['values']
									);
					}
	...
	public function validateFile($value, $options, $field_label = '', $internalname) {
		...
		if (!empty($options['mimetype']) && !empty($file['type'])) {
			...
			if ($success_mime == FALSE) {
				$result['status'] = false;
				$result['message'] = elgg_echo('gtask_ktform:validation:file:mimetype');
				return $result;
			}
		}
		...
		if (!empty($options['max_size'])) {
			if (in_array('size', $file)) {
				if ($file['size'] > $options['max_size']) {
					$result['status'] = false;
					$result['message'] = elgg_echo('gtask_ktform:validation:file:maxfilesize');
	...
	public function validateDate_range($value, $options, $field_label = '', $internalname) {
		...
		$current_date = GtaskBaseMain::ktform_get_default_dates();
		$valid_range = FALSE;
		...
		if ($valid_range == FALSE) {
			$result['status'] = false;
			$error = elgg_echo('gtask_ktform:validate:date_range:error');
	...
	public function validatePeriod_range($value, $options, $field_label = '', $internalname) {
		...
		if($valid_range_required) {
		    ...
		    if(!$valid_range) {
			$error_custom .= sprintf(elgg_echo('gtask_ktform:validate:period_range:error:date'), 
			...
			switch($key_name) {
			    ...
			    case 'month_to':
					...
					$valid_range = FALSE;
					$error_custom .= sprintf(elgg_echo('gtask_ktform:validate:period_range:error:month'), $period_valid_fields['month_min'], $period_valid_fields['month_max']);
				    }
				break;
			    case 'year_from':
			    case 'year_to':
					...
					$valid_range = FALSE;
					$error_custom = sprintf(elgg_echo('gtask_ktform:validate:period_range:error:year'), $period_valid_fields['year_min'], $period_valid_fields['year_max']);
				    }
		...
		if (!$valid_range_required || !$valid_range) {
			$result['status'] = false;
			$error = elgg_echo('gtask_ktform:validate:period_range:error') . $error_custom;
			$result['message'] = $error;
			return $result;
		} 
	...
	public function validateUrl($value, $option, $field_label, $field_internalname) {
		...
		if(!filter_var($value, FILTER_VALIDATE_URL)) {
			$result['status'] = FALSE;
			$result['message'] = elgg_echo('gtask_ktform:validation:invalid:url');
		}
	...
	public function validateVideo($value, $option, $field_label, $field_internalname) {
		...
		if(is_callable(array('GtaskBaseUrlFeed', 'validateUrl'))) {
			$urlFeed = new GtaskBaseUrlFeed();
			
			$type = $urlFeed->validateUrl($value);
			if(!$type) {
				$sites = implode(', ', $urlFeed->getSupportedSites());
				$result['status'] = false;
				$result['message'] = sprintf(elgg_echo('gtask_ktform:validation:invalid:video:url'), $sites);
			}
		}
	...
	public function validateNon_empty_array($value, $required = FALSE, $field_label = '') {
		...
		if (is_array($value)) {
			$filtered = array_filter($value, 'GtaskRemoveEmptyItems');
		} else {
		...
		if($required && empty($filtered)) {
			$result['status'] = false;
			$result['message'] = elgg_echo('gtask_ktform:validation:array:required:text');
		}
 ...
 * @return array 
 */
function GtaskRemoveEmptyItems($item) {
	if (is_array($item)) {
