...
/*
 * This is a class from izap videos, with some modifications to be used into gtask_ktform.
 * Thank you Izap Guys. :)
 */
  ...
  function &asPostString(&$theData, $theName = NULL) {
    ...
    if (is_array($theData)) {
      foreach ($theData as $theKey => $theValue) {
        if ($thePrefix === NULL) {
          $thePostString .= '&' . GtaskBaseCurl::asPostString($theValue, $theKey) ;
        }
        else {
          $thePostString .= '&' . GtaskBaseCurl::asPostString($theValue, $thePrefix . '[' . $theKey . ']') ;
        }
      }
    }
    else {
...
class gtask_xml2array {
  function gtask_xml2array( $xml ) {
    if ( file_exists($xml) )
