...
?>
<div>
	<label><?php echo elgg_echo("profile:{$shortname}") ?> <?php echo (in_array($shortname, $required_fields))? ' *':''; ?></label>
	<?php
		$params = array(
			'name' => $shortname,
			'value' => $value,
			'class' => "profile-custom-input-{$shortname}",
		);
...
<?php elgg_clear_sticky_form('profile:edit'); ?>