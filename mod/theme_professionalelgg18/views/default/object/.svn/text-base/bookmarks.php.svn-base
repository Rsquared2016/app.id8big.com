...
$date =  '<span class="addDot">' . elgg_view_friendly_time($bookmark->time_created) . '</span>';
...
<div class="bookmark elgg-content mts">
	<span class="elgg-heading-basic">$link</span>
	$description
</div>
HTML;
...
HTML;
	...
	$display_text = $url;
	$excerpt = elgg_get_excerpt($bookmark->description, 110); // only two rows
	if ($excerpt) {
		$excerpt = " - $excerpt";
	}
