...
$page_icon = elgg_view_entity_icon($page->getOwnerEntity(), 'tiny');
...
$date =  '<span class="addDot">' . elgg_view_friendly_time($annotation->time_created) . '</span>';
$editor_text = elgg_echo('pages:strapline', array($date, $editor_link));
//$tags = elgg_view('output/tags', array('tags' => $page->tags));
$categories = elgg_view('output/categories', $vars);

$revision_url =  '<span class="addDot">' . elgg_view('output/url', array('text' => elgg_echo('pages:revision'), 'href' => $annotation->getURL(), 'class' => 'pagesRevisionUrl')) . '</span>';
...
if ($comments_count != 0 && !$revision) {
	...
	$comments_link = '<span class="addDot">' . $comments_link . '</span>';
}
else {
	$comments_link = '';
}
...
//		'tags' => $tags,
	...
	// brief view
	$excerpt = elgg_get_excerpt($page->description, 150); // only two rows
	$params = array(
		'entity' => $page,
		'metadata' => $metadata,
		'subtitle' => $subtitle,
