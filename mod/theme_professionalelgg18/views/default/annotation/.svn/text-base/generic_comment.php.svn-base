...
$friendlytime = '<span class="friendlyTimeFormat">' . elgg_view_friendly_time($comment->time_created) . '</span>';

$the_context = elgg_get_context();

if($the_context == 'activity') {
	$avatar_size = 'tiny';
}
else {
	$avatar_size = 'small';
...
$commenter_icon = elgg_view_entity_icon($commenter, $avatar_size);
...
<div class="mbn">
	<div class="commentTop">
		<div class="commentTopInner flLef">
			$commenter_link
			<span class="elgg-subtext nm">
				$friendlytime
			</span>
		</div>
		$menu
...
HTML;
	...
	$on = '<span class="itsOn">' . elgg_echo('on'). '</span>';
