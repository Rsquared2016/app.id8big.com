...
jQuery.fn.friendsPicker = function(iterator) {
	...
	return this.each(function() {
		...
		var panelWidth = 0;
		var panelCount = 0;
		var friendsPicker_containerWidth = 0;

		function resize_friends_picker() {

			// set panelwidth manually as it's hidden initially - adjust this value for different themes/pagewidths
			panelWidth = $('.elgg-main.elgg-body').width();
			...
			friendsPicker_containerWidth = panelWidth * panelCount;
			...
			// update panel widths
			$('.friends-picker, .friends-picker .friends-picker-container .panel').width(panelWidth);
			// update main container position
			$('.friends-picker-container').css({'left' : -((currentPanel - 1) * panelWidth) + 'px'});
			// update arrows position
			$('.friends-picker-navigation-l, .friends-picker-navigation-r').css({'top' : ($('.friends-picker-navigation').height() + 21) + 'px'});
		...
		// resize / reposition on window resize
		$(window).resize(
			function() {
				resize_friends_picker();
			}
		);
