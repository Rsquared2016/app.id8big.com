...
$image_support = NewsBaseMain::ktform_get_entity_image_support($subtype);
...
$icon = FALSE;
if ($image_support && ($image_support !== NewsBaseMain::$News_st_OWNER_ICON_NAME)) {
//Get icon
...
<div class='ktFormCont'>
		...
		<?php if (is_array($object_fields) && count($object_fields) > 0) { ?>
			<ul class='ktFormFields ktFormProfile'>
				...
				foreach ($object_fields as $internalname => $field_data) {
					$tmp_classname = NewsBaseMain::ktform_camelize_string($internalname, FALSE);
	...
	$profile_btns = elgg_view('news/profile/buttons', $vars);
	...
	<div class='ktFormDescCont'>
		<?php foreach ($object_description_fields as $desc_internalname => $description_field) { ?>
			<div class="ktFormDescInner <?php echo NewsBaseMain::ktform_camelize_string($desc_internalname, FALSE) ?>">
	...
	<?php
	echo elgg_view('news/profile/profile_footer', $vars);
	?>
</div>