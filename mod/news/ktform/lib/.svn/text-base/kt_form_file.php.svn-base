...
/**
 * This library pretend to save files to one object handled with news_ktform input file
 */
//Constant that defines the file name, that will handle the session and the filefactory
define('News_st_FILE_NAME', 'kt_file');

function news_file_init() {
	/**
	 * Register the events
	 */
//	elgg_register_event_handler('create', 'user', 'news_file_hander', 999);
	elgg_register_event_handler('profileupdate', 'user', 'news_file_hander', 999);
	
	elgg_register_event_handler('create', 'object', 'news_file_hander', 999);
	elgg_register_event_handler('update', 'object', 'news_file_hander', 999);
	
	elgg_register_event_handler('update', 'group', 'news_file_hander', 999);
	elgg_register_event_handler('update', 'group', 'news_file_hander', 999);

	elgg_register_page_handler('kt_file', 'news_files_handler');

	elgg_register_action('news/download/file', dirname(dirname(__FILE__)) . '/actions/kt_file/download_a.php', TRUE);
}

function news_files_handler($page) {
	switch ($page[0]) {
		default:
			if (isset($page[0])) {
...
function news_file_hander($event, $object_type, $object) {
	...
	if ($check_event && $check_object) {
		...
		if(!array_key_exists(News_st_FILE_NAME, $_SESSION)) {
			return TRUE;
		}
		 ...
		 * and object subtype
		 */
		$KT_FILE = $_SESSION[News_st_FILE_NAME];
		if (!is_array($KT_FILE)) {
			return TRUE;
		}
		...
		unset($_SESSION[News_st_FILE_NAME]);

		$owner_guid = $object->getOwnerGUID();
		$object_guid = $object->getGUID();

		$prefix = news_file_get_prefix($object);
		...
		foreach ($file_inputs as $internalname => $input) {
			if (isset($_FILES[$internalname]['name']) && !empty($_FILES[$internalname]['name'])) {
				...
				$existent_filename = news_get_filestore_data($object, $internalname, 'filename');
				...
				$filestore_save_data = array(
					...
					'simpletype' => news_get_general_file_type($_FILES[$internalname]['type']),
					'size' => $file->size(),
				);
...
function news_file_get_prefix($object) {
	...
	$prefix = $prefix . DIRECTORY_SEPARATOR . News_st_FILE_NAME;

	return $prefix;
}

function news_get_filestore_data($object, $internalname, $return_key = FALSE) {
 ...
 */
function news_get_general_file_type($mimetype) {
...
elgg_register_event_handler('init', 'system', 'news_file_init');