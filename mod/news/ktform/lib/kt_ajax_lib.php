 ...
 * @package    NewsBaseForm
 * @subpackage news_ajax
 */
 ...
 * Init the ajax 
 */
function news_ajax_init() {
	elgg_register_page_handler('search_endpoint', 'news_ajax_endpoint_handler');
	elgg_extend_view('page/elements/head', 'metatags/jquery_autocomplete', 1000);

	elgg_register_plugin_hook_handler('search', 'countries_short', 'news_search_countries_short');

	elgg_register_plugin_hook_handler('search_types', 'get_types', 'news_search_register_country_types');
}

function news_ajax_endpoint_handler($page = array()) {
 ...
 *  
 */
function news_search_get_registered_entity_types() {
 ...
 *  return array
 */
function news_search_get_filters_list() {
	$filter_list = array();

	$types = news_search_get_registered_entity_types();
...
function news_search_extract_src_from_img_tag($img_tag) {
	$values = array();
	$src = '';
...
function news_search_countries_short($hook, $object_type, $returnvalue, $params) {
	$search_type = FALSE;
	...
	$all_countries = NewsBaseMain::ktform_get_country_values();
...
function news_search_register_country_types($hook, $object_type, $returnvalue, $params) {
	$returnvalue['countries_short'] = 'countries_short';
	return $returnvalue;
}

elgg_register_event_handler('init', 'system', 'news_ajax_init');