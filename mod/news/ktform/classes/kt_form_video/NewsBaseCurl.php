...
/*
 * This is a class from izap videos, with some modifications to be used into news_ktform.
 * Thank you Izap Guys. :)
 */
  ...
  function &asPostString(&$theData, $theName = NULL) {
    ...
    if (is_array($theData)) {
      foreach ($theData as $theKey => $theValue) {
        if ($thePrefix === NULL) {
          $thePostString .= '&' . NewsBaseCurl::asPostString($theValue, $theKey) ;
        }
        else {
          $thePostString .= '&' . NewsBaseCurl::asPostString($theValue, $thePrefix . '[' . $theKey . ']') ;
        }
      }
    }
    else {
...
class news_xml2array {
  function news_xml2array( $xml ) {
    if ( file_exists($xml) )
