...
'.$url;
			...
			$response = $this->api->message($options['recipients'], $options['subject'], $options['body'], $options['copy_self']);
				...
				if (is_array($recipients) && is_array($recipients_names)) {
					foreach ($recipients as $rec) {
						if (array_key_exists($rec, $recipients_names)) {
							...
							$opt_annot = array(
								'guid' => $user->getGUID(),
								'annotation_names' => SOCIAL_IMPORT_CONTACTS_INVITED_CONTACTS . '_linkedin',
