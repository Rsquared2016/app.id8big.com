...
<table width="100%" border="0">
  ...
  <tr>
    <td align="center"><br /><h3><?php echo elgg_echo('elgg_social_login:loading'); ?></h3><br /></td> 
  </tr>
  <tr>
...
<?php
	...
	if( isset( $_GET["provider"] ) && isset( $_GET["redirect_to_provider"] )){
		try{ 
			...
			if( ! elgg_get_plugin_setting( 'ha_settings_' . $provider . '_enabled', 'elgg_social_login' ) ){
			...
			if ( ! $users ) { 
				...
				if ($birthday && $birthmonth && $birthyear) {
					$user->birthday = "{$birthday}/{$birthmonth}/{$birthyear}";
				}
				...
				// notice && login
				system_message( elgg_echo('elgg_social_login:created:account', array($provider)) );
				login( $user ); 
			...
			# {{{ user image
				if( $user_profile->photoURL ){ 
					$photoURL = str_replace('square', 'large', $user_profile->photoURL);
			...
			elseif (count($users) == 1) { 
				...
				// notice
				system_message( elgg_echo('elgg_social_login:signed:success', array($provider)) );
			}
			else{
