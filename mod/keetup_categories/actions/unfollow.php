...
try {
	...
	foreach ($guids as $guid) {
		...
		if($entity instanceof $instance) {
			//Follow the entity.
			$success = remove_entity_relationship($user_guid, $follow_rel, $guid);
			...
			if(!$success) {
				throw new Exception(sprintf(elgg_echo('keetup_categories:unfollow:error:removing:rel'), $type_text));
			}
		} else {
...
if($success) {
	...
	system_message(sprintf(elgg_echo("keetup_categories:unfollow:success"), $type_text));
}
