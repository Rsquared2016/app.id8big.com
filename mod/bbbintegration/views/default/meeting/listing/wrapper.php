...
if (isset($vars['extra_class']) && !empty($vars['extra_class'])) {
	...
	if (!empty($entity_subtype)) {
		...
		$extra_class = MeetingBaseMain::ktform_camelize_string($entity_subtype, false);
...
$error_msg = elgg_echo("meeting_ktform:entities:{$entity_subtype}:listing:empty_results");
...
<div class="ktBulkActionsBottom">
	<?php
		echo elgg_view('meeting/bulk_actions/form', $vars);
	?>
</div>
<?php
}
