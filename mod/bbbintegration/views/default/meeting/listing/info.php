...
//meeting_ktform_get_trimed_title().
$title = $entity->title;
if (!$title) {
	$title = $entity->name;
}
...
if(strlen($title) > Meeting_st_MAX_LENGHT_TITLE_LISTING){
	$title = elgg_get_excerpt($title, Meeting_st_MAX_LENGHT_TITLE_LISTING);
}
?>
...
?>
	<h4>
		...
		<?php
			...
			echo elgg_view('meeting/listing/info_middle_extend', $vars);
...
<?php
	 ...
	 * */
	$extra_fields = MeetingBaseMain::ktform_get_extra_listing_fields($entity_subtype);
 ...
 * */
//echo elgg_view('meeting/tools/action_wheel', $vars);
