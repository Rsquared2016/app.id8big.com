 ...
 */
/**
 * meeting_ktform_get_default_dates return an array with k=>v the value is the current start time and end time of key, calendar_start and calendar_end 
 */
$entity = FALSE;
...
$defaults = MeetingBaseMain::ktform_get_default_dates(FALSE);
...
$calendar_options[] = 'buttonImage: "' . $vars['url'] . 'mod/bbbintegration/ktform/graphics/custom/events/ico-calendar.png"';
...
//KTODO: Add all the logic of intervaltimes into meeting_ktform.
...
if($vars['intervaltimes'] && elgg_is_active_plugin('meetups')) {
	...
	if ($vars['calendar_end'] != FALSE) {
		echo '<span class="dateRangeTo">'. elgg_echo('meeting_ktform:input:date:range:to') .'</span>';
	}
