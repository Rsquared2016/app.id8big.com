...
$entity_subtype = MeetingBaseMain::ktform_get_subtype($vars);

$image_support = MeetingBaseMain::ktform_get_entity_image_support($entity_subtype);
...
if ($image_support) {
	...
	if ($image_support === MeetingBaseMain::$Meeting_st_OWNER_ICON_NAME) {
...
//$kt_chck = defined('Meeting_st_DISABLE_BULK_ACTIONS') && Meeting_st_DISABLE_BULK_ACTIONS != TRUE; // yes, there is one!
$kt_chck = MeetingBaseMain::ktform_get_bulk_action_support($entity_subtype);
if ($kt_chck && $entity) {
	$class_chck = 'chckCont';
	if (!$icon) {
...
$owner_link = elgg_view('output/url', array(
	'href' => "meeting/owner/$owner->username",
	'text' => $owner->name,
	'is_trusted' => true,
));
...
$subtitle = $subtitle.elgg_view('meeting/listing/subtitle_extended', $vars);
...
$tags = $tags.elgg_view('meeting/listing/tags_extended', $vars);
...
// Get start datetime
$start_datetime = elgg_view('meeting/listing/start_datetime', $vars);
...
$params = array(
	 ...
	 'metadata' => $metadata,
	 'subtitle' => $subtitle,
	 'content' => $start_datetime.$description,
	 'tags' => $tags,
);
