...
function meeting_images_init() {
	global $CONFIG;

	elgg_extend_view('css/elgg','images/css');

	elgg_register_page_handler('meeting_icon','meeting_images_page_handler');
	
	elgg_register_event_handler('create', 'object', 'meeting_images_image_hander');
	elgg_register_event_handler('update', 'object', 'meeting_images_image_hander');
	
//	elgg_register_event_handler('create', 'group', 'meeting_images_image_hander');
//	elgg_register_event_handler('update', 'group', 'meeting_images_image_hander');
	
	
	// Now override icons
	elgg_register_plugin_hook_handler('entity:icon:url', 'all', 'meeting_images_icon_hook', 999);
}


function meeting_images_page_handler($page) {
	global $CONFIG;
	...
	include $CONFIG->pluginspath . 'bbbintegration/ktform/pages/images/thumb.php';

	return TRUE;
}

function meeting_images_image_hander($event, $object_type, $object) {
...
//			'group',
	...
	if($check_event && $check_objects && $object->getType() == 'object' && $object->getSubType() == 'meeting') {
		...
		if(isset($_SESSION['images']['input_image'])) {
			...
			if (isset($_FILES[$input_image]) && $_FILES[$input_image]['error'] == 0) {
				...
				$prefix = meeting_images_get_prefix($object);
				...
				if(isset($image_options['resize']) && $image_options['resize']) {
					...
					$icon_sizes = elgg_trigger_plugin_hook('meeting_images:icon_sizes', 'object', array('entity' => $object), $icon_sizes);
					...
					foreach ($icon_sizes as $name => $size_info) {
						...
						$resized = meeting_images_get_resized_image_from_existing_file($_FILES[$input_image]['tmp_name'], $size_info['w'], $size_info['h'], $size_info['square'], 0, 0, 0, 0, $size_info['upscale']);
...
function meeting_images_icon_hook($hook, $entity_type, $returnvalue, $params){
	global $CONFIG;

	if ((!$returnvalue) && ($hook == 'entity:icon:url') && ($params['entity'] instanceof Meeting)) {
		...
		$prefix = meeting_images_get_prefix($entity);
		...
		if($file_exists) {
			$url = $CONFIG->url . "meeting_icon/$prefix/$entity_guid/$size/$icontime.jpg";
			return $url;
		}
	}
}

function meeting_images_get_entity_icon_url($entity, $size, $viewtype = '') {
	...
	return meeting_images_icon_hook($hook, $entity->getType(), '', $params);
}


function meeting_images_get_prefix($object) {
	$otype = $object->type;
 ...
 */
function meeting_images_get_resized_image_from_existing_file($input_name, $maxwidth, $maxheight, $square = FALSE, $x1 = 0, $y1 = 0, $x2 = 0, $y2 = 0, $upscale = FALSE) {
...
elgg_register_event_handler('init', 'system', 'meeting_images_init');