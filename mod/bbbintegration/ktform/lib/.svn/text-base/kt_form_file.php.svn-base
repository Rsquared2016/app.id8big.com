...
/**
 * This library pretend to save files to one object handled with meeting_ktform input file
 */
//Constant that defines the file name, that will handle the session and the filefactory
define('Meeting_st_FILE_NAME', 'kt_file');

function meeting_file_init() {
	/**
	 * Register the events
	 */
//	elgg_register_event_handler('create', 'user', 'meeting_file_hander', 999);
	elgg_register_event_handler('profileupdate', 'user', 'meeting_file_hander', 999);
	
	elgg_register_event_handler('create', 'object', 'meeting_file_hander', 999);
	elgg_register_event_handler('update', 'object', 'meeting_file_hander', 999);
	
	elgg_register_event_handler('update', 'group', 'meeting_file_hander', 999);
	elgg_register_event_handler('update', 'group', 'meeting_file_hander', 999);

	elgg_register_page_handler('kt_file', 'meeting_files_handler');

	elgg_register_action('meeting/download/file', dirname(dirname(__FILE__)) . '/actions/kt_file/download_a.php', TRUE);
}

function meeting_files_handler($page) {
	switch ($page[0]) {
		default:
			if (isset($page[0])) {
...
function meeting_file_hander($event, $object_type, $object) {
	...
	if ($check_event && $check_object) {
		...
		if(!array_key_exists(Meeting_st_FILE_NAME, $_SESSION)) {
			return TRUE;
		}
		 ...
		 * and object subtype
		 */
		$KT_FILE = $_SESSION[Meeting_st_FILE_NAME];
		if (!is_array($KT_FILE)) {
			return TRUE;
		}
		...
		unset($_SESSION[Meeting_st_FILE_NAME]);

		$owner_guid = $object->getOwnerGUID();
		$object_guid = $object->getGUID();

		$prefix = meeting_file_get_prefix($object);
		...
		foreach ($file_inputs as $internalname => $input) {
			if (isset($_FILES[$internalname]['name']) && !empty($_FILES[$internalname]['name'])) {
				...
				$existent_filename = meeting_get_filestore_data($object, $internalname, 'filename');
				...
				$filestore_save_data = array(
					...
					'simpletype' => meeting_get_general_file_type($_FILES[$internalname]['type']),
					'size' => $file->size(),
				);
...
function meeting_file_get_prefix($object) {
	...
	$prefix = $prefix . DIRECTORY_SEPARATOR . Meeting_st_FILE_NAME;

	return $prefix;
}

function meeting_get_filestore_data($object, $internalname, $return_key = FALSE) {
 ...
 */
function meeting_get_general_file_type($mimetype) {
...
elgg_register_event_handler('init', 'system', 'meeting_file_init');