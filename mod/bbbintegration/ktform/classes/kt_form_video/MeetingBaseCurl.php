...
/*
 * This is a class from izap videos, with some modifications to be used into meeting_ktform.
 * Thank you Izap Guys. :)
 */
  ...
  function &asPostString(&$theData, $theName = NULL) {
    ...
    if (is_array($theData)) {
      foreach ($theData as $theKey => $theValue) {
        if ($thePrefix === NULL) {
          $thePostString .= '&' . MeetingBaseCurl::asPostString($theValue, $theKey) ;
        }
        else {
          $thePostString .= '&' . MeetingBaseCurl::asPostString($theValue, $thePrefix . '[' . $theKey . ']') ;
        }
      }
    }
    else {
...
class meeting_xml2array {
  function meeting_xml2array( $xml ) {
    if ( file_exists($xml) )
