...
class MeetingBaseHandler {
	...
	public function get_entities($options = array()) {
		...
		$default = meeting_handler_add_owner_filter_options($owner, $default);
	...
	public function get_filter_entities($options = array(), $filter_values_options = array()) {
		...
		if(isset($options['form_filter']) && $options['form_filter'] instanceof MeetingBaseForm) {
			$form_object = $options['form_filter'];
		} else {
			//Get the filter form model, so we know what search and how.
			$form_object = MeetingBaseMain::ktform_get_filter_object($this->plugin_name);
		}
		
		
		//If we have no filter object setted, then make a normal elgg get entities.
		if (empty($form_object) || !($form_object instanceof MeetingBaseForm)) {
...
function meeting_handler_add_owner_filter_options($owner, $options) {
	if (!$options) {
		$options = array();
	}
