...
function elgg_timezone_setter() {
    ...
    $site_timezone = elgg_get_plugin_setting('site_timezone', 'bbbintegration');
    $user_timezone = elgg_get_plugin_user_setting('user_timezone', 0, 'bbbintegration');
 ...
 *  
 */
function elgg_timezone_get_timezones($is_pulldown = FALSE) {
...
//                . elgg_timezone_format_offset($row['offset'])
	...
	if ($is_pulldown) {
		$options = array('0' => elgg_echo('meeting:timezone:pulldown:label')) + $options;
	}
...
function elgg_timezone_set_timezone_user(ElggUser $user, $timezone = '') {
	...
	if ($valid_user && $valid_timezone) {
		...
		$return_value = elgg_set_plugin_user_setting('user_timezone', $timezone, $user_guid, 'bbbintegration');
	}
...
function elgg_timezone_get_timezone_user(ElggUser $user) {
	...
	if (elgg_instanceof($user, 'user')) { 
		$value = elgg_get_plugin_user_setting('user_timezone', $user->getGUID(), 'bbbintegration');
		return $value;
	}
...
/**
 * Get a specific timezone for site
 * 
 * @param ElggUser $user
 * @return boolean 
 */
function elgg_timezone_get_timezone_site() {
    
	$value = elgg_get_plugin_setting('site_timezone', 'bbbintegration');
