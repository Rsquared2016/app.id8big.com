 ...
 * @version 1.0
 * 
 * Adapted for Elgg by Kling Emanuel and Scarel German
 * 
 * Server Demo
...
class BigBlueButton {
    ...
    public function __construct() {
        ...
        // Set url
        $this->url = elgg_get_plugin_setting('server_url', 'bbbintegration');
        
        // Set salt
        $this->salt = elgg_get_plugin_setting('security_salt', 'bbbintegration');
        
        // Set logout url
        $this->logoutUrl = elgg_get_site_url();
    ...
    public function getCreateMeetingUrl($name, $params = array()) {
        ...
        $url = $this->createApiQuery("create", $params);
    ...
    public function getJoinMeetingUrl($meetingId, $username, $userId = null, $role = self::ROLE_VIEWER, $params = array()) {
        ...
        if (empty($userId) && elgg_is_logged_in()) {
            $params['userID'] = elgg_get_logged_in_user_guid();
        }
        else {
        ...
        $url = $this->createApiQuery("join", $params);
    ...
    public function endMeeting($meetingId, $password = null) {
        ...
        $url = $this->getEndMeetingUrl($meetingId, $password == null ? $this->moderatorPW : $password);
    ...
    public function meetingIsRunning($meetingId) {
        ...
        $url = $this->createApiQuery("isMeetingRunning", array("meetingID" => $meetingId));
    ...
    protected function processMeetings($data, $state, $requestInfo = true, $userId = null, $username = null, $role = null) {
        ...
        if (!isset($data['meetings']) || !isset($data['meetings']['meeting'])) {
            return $meetings;
        }
        ...
        if (isset($data['meetings']['meeting']['meetingID'])) {
            $result = $this->processMeeting(
                    $data['meetings']['meeting'], $state, $requestInfo, $userId, $username, $role
            );
            if (!$result == false) {
                $meetings[] = $result;
            }
            ...
            foreach ($data['meetings']['meeting'] as $meeting) {
                $result = $this->processMeeting(
                        $meeting, $state, $requestInfo, $userId, $username, $role
                );
                if ($result === false) {
                    continue;
                }
    ...
    protected function processUserMeeting($meetingData, $userId, $username, $role) {
        ...
        if ($role == self::ROLE_MODERATOR ||
            ...
            $endUrl = $this->getEndMeetingUrl($meetingData['meetingID'], $pass);
        }
        else {
        ...
        if (!$currentUserJoined) {
            $joinUrl = $this->getJoinMeetingUrl($meetingData['meetingID'], $username, $userId, $role, array("password" => $pass));
        }
