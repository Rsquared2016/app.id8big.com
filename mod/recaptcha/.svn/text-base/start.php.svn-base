...
define('RECAPTCHA_CUSTOM_THEME', 'red');
...
function recaptcha_init() {
	...
	elgg_register_plugin_hook_handler('action', 'user/requestnewpassword', 'recaptcha_check_form');

    // unset the validated recaptcha session variable
    elgg_register_plugin_hook_handler('register', 'user', 'recaptcha_unset_session');
    elgg_register_css('recaptcha', 'mod/recaptcha/css/captcha.css');

    elgg_register_admin_menu_item('configure', 'recaptcha', 'appearance', 40);
    elgg_register_action('recaptcha/theme_selector', dirname(__FILE__).'/actions/recaptcha/theme_selector_a.php', 'admin');

//	elgg_extend_view('register/extend', 'input/captcha');
}

function recaptcha_get_custom_theme() {
    $default_theme = RECAPTCHA_CUSTOM_THEME;
    $setting_theme = elgg_get_plugin_setting('theme', 'recaptcha');
