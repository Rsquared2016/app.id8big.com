...
if(!elgg_is_active_plugin('izap_videos')) {
    require_once  $kt_form_class_path. '/classes/kt_form_video/EventsBaseCurl.php';
}

//Get Feed.
require_once  $kt_form_class_path. '/classes/kt_form_video/EventsBaseGetFeed.php';
require_once  $kt_form_class_path. '/classes/kt_form_video/EventsBaseUrlFeed.php';

function events_videos_init() {
	global $CONFIG;

	elgg_register_event_handler('create', 'object', 'events_videos_video_hander');
	elgg_register_event_handler('update', 'object', 'events_videos_video_hander');
	
	// Now override icons
	//elgg_register_plugin_hook_handler('entity:icon:url', 'object', 'events_videos_icon_hook', 998);
	
	elgg_register_plugin_hook_handler('ktform:profile_full', 'object', 'events_videos_profile_full_hook');
}


function events_videos_video_hander($event, $object_type, $object) {
		...
		if(isset($_SESSION['videos']['input_video'])) {
			...
			if(is_array($video_components) && isset($video_components['error'])) {
				...
				if($video_image && $video_options['generate_thumb']) {
					...
					$icon_sizes = elgg_trigger_plugin_hook('events_videos:icon_sizes', 'object', array('entity' => $object), $icon_sizes);
...
function events_videos_profile_full_hook($hook, $entity_type, $returnvalue, $params){
	global $CONFIG;
	...
	if ($hook == 'ktform:profile_full'){
		...
		if($entity->video_type && $entity->video_thumbnail) {
			...
			return elgg_view('events/video', $params);
		}
	}
}
...
function events_videos_icon_hook($hook, $entity_type, $returnvalue, $params){
	global $CONFIG;
...
elgg_register_event_handler('init', 'system', 'events_videos_init');