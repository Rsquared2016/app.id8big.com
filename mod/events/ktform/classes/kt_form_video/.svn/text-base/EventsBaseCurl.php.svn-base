  ...
  function &asPostString(&$theData, $theName = NULL) {
    ...
    if (is_array($theData)) {
      foreach ($theData as $theKey => $theValue) {
        if ($thePrefix === NULL) {
          $thePostString .= '&' . EventsBaseCurl::asPostString($theValue, $theKey) ;
        }
        else {
          $thePostString .= '&' . EventsBaseCurl::asPostString($theValue, $thePrefix . '[' . $theKey . ']') ;
        }
      }
    }
    else {
...
class events_xml2array {
  function events_xml2array( $xml ) {
    if ( file_exists($xml) )
