 ...
 */
define('Events_st_INLINE_GROUP_STRING', 'inline');

/**
 * EventsBaseForm class
 * 
 ...
 * @author Diego Gallardo and German Bortoli
 */
class EventsBaseForm {

	private static $BUNDLE_NAME = 'events';
...
//		$this->setSaver('EventsBaseSave');

		$this->setValidator('EventsBaseValidators');
	 ...
	 * 	by default it will return EventsBaseSave string, if none is setted.
	 * 
	 * @return string 
	 */
	 ...
	 * 	by default will return EventsBaseValidators string, if none is setted
	 * 
	 * @return string 
	 */
	 ...
	 * @param string $class the EventsBaseForm class name
	 * 
	 * @return Instance EventsBaseSave | FALSE if no success 
	 */
	...
	public function setTypes($type = 'object', $subtype = '') {
		...
		switch ($this->type) {
			case 'group':
				$this->setSaver('EventsBaseGroupSave');
				break;
			default:
				$this->setSaver('EventsBaseSave');
				break;
	...
	public function setObject($object_value) {
		...
		if (!$this->__isNewEntity()) {
			if (($object->getSubtype() != $this->getSubType()) || $this->getType() != $object->getType()) {
				throw new EventsBaseInputException(elgg_echo('events:error:no_equal_subtype_and_type'));
			}
		}
	...
	public function setInput($input_options = array()) {
		...
		if (!(array_key_exists('internalname', $input_options) && !empty($input_options['internalname']))) {
			throw new EventsBaseInputException(elgg_echo('events:error:empty:internalname'));
		}

		if (!(array_key_exists('type', $input_options) && !empty($input_options['type']))) {
			throw new EventsBaseInputException(elgg_echo('events:error:empty:type'));
		}
		...
		if (!empty($input_options['label'])) {
			if (is_bool($input_options['label'])) {
				$input_options['label'] = elgg_echo("events:{$this->getPluginName()}:label:{$input_options['internalname']}");
			} else {
		...
		if ($input_options['info_text']) {
			if (is_bool($input_options['info_text'])) {
				$input_options['info_text'] = elgg_echo("events:{$this->getPluginName()}:info_text:{$input_options['internalname']}");
			} else {
	...
	public function setFields($form_fields = array()) {
		...
		$form_fields = elgg_trigger_plugin_hook('events:fields', $plugin_hook_name, NULL, $form_fields);
	...
	public function setFormVars($form_vars = array()) {
		...
		$defaults = array(
			 ...
			 'js' => 'onsubmit="javascript:return events_ktform_process_submit(this);"',
			 'internalid' => '',
			 'internalname' => '',
			 'enctype' => '',
		...
		$custom_rel = Events_st_AJAX_REL_ATTR;
	...
	/**
	 * Set the css classname of the container div inside the view input/events_submit.
	 * 
	...
	public function renderFieldsToArray($render_options = array()) {
		...
		$submit_text_elgg = "events:{$plugin_name}:label:submit:send";
		...
		foreach ($form_fields as $field) {
			...
			if ($field['type'] == 'text') {
				if (is_array($field['options'])) {
					if ($field['internalname'] == 'title') {
						$js_max_length_input = 'maxlength=' . Events_st_MAX_LENGHT_TITLE;
					} else {
						$js_max_length_input = 'maxlength=' . Events_st_MAX_LENGHT_TEXT;
					}
		...
		foreach ($form_fields as $internalname => $field) {
			...
			if ($object_value != '') {
				...
				switch ($type) {
					...
					case 'dropdown':
					case 'pulldown':
						$options['value'] = EventsBaseMain::get_values_for_outputs_with_options($options, FALSE);
						break;
					case 'checkboxes':
						$options['value'] = EventsBaseMain::get_values_for_outputs_with_options($options);
						break;
...
//						break;
	...
	public function render($render_options = array(), $before_footer_string = '') {
		...
		$body_form_class = EventsBaseMain::ktform_camelize_string($this->getPluginName(), FALSE);
		...
		$extraClass = '';
		if (elgg_get_plugin_setting('profile_label_above', 'events') == 'yes') {
		...
		foreach ($rendered_group_fields as $rendered_group => $rendered_fields) {
			...
			$is_inline = (bool) strstr($rendered_group, Events_st_INLINE_GROUP_STRING);
			$title = '';
	...
	public function save($register_errors = TRUE, $override_data = array()) {
		...
		if ($success != FALSE && ($type || $subtype)) {
			...
			$guid = $this->getObject();
			//$saver = new EventsBaseSave($type, $subtype, $class);
...
//			}
	...
	private function __setSessionField($internalname, $value) {
		...
		if ($value != '') {
			$_SESSION['events'][$this->getPluginName()] = $this->session;
		}
	...
	private function __getSessionValue($internalname) {
		$session = $_SESSION['events'][$this->getPluginName()];
	...
	private function __getSessions() {
		return $_SESSION['events'][$this->getPluginName()];
	}
	...
	public function __clearSession() {
		...
		$_SESSION['events'][$this->getPluginName()] = NULL;
		unset($_SESSION['events'][$this->getPluginName()]);
	}
