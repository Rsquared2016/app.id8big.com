...
class EventsBaseHandler {
	...
	public function get_entities($options = array()) {
		...
		$default = events_handler_add_owner_filter_options($owner, $default);
	...
	public function get_filter_entities($options = array(), $filter_values_options = array()) {
		...
		if(isset($options['form_filter']) && $options['form_filter'] instanceof EventsBaseForm) {
			$form_object = $options['form_filter'];
		} else {
			//Get the filter form model, so we know what search and how.
			$form_object = EventsBaseMain::ktform_get_filter_object($this->plugin_name);
		}
		
		
		//If we have no filter object setted, then make a normal elgg get entities.
		if (empty($form_object) || !($form_object instanceof EventsBaseForm)) {
...
function events_handler_add_owner_filter_options($owner, $options) {
	if (!$options) {
		$options = array();
	}
