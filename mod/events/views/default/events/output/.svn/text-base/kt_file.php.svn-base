...
$filestore_data = events_get_filestore_data($entity, $file);
...
if ($filestore_data) {
	if (isset($vars['is_action']) && ($vars['is_action'] == TRUE)) {
		...
		$href = $vars['url']."events_kt_file/{$entity->getGUID()}/{$file}/";
...
if (empty($vars['text'])) {
	$vars['text'] = elgg_echo('events_file:download_link');
}
