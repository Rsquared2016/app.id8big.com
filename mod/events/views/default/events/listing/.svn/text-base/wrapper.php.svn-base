...
if (isset($vars['extra_class']) && !empty($vars['extra_class'])) {
	...
	if (!empty($entity_subtype)) {
		...
		$extra_class = EventsBaseMain::ktform_camelize_string($entity_subtype, false);
...
$css = '';
$extra_fields = EventsBaseMain::ktform_get_extra_listing_fields($entity_subtype);
...
$error_msg = elgg_echo("events_ktform:entities:{$entity_subtype}:listing:empty_results");
...
<div class="ktBulkActionsBottom">
	<?php
		echo elgg_view('events/bulk_actions/form', $vars);
	?>
</div>
<?php
}
