...
$exclude_fields = array('title', 'description', 'access_id', 'container_guid', 'all_day');
...
$image_support = EventsBaseMain::ktform_get_entity_image_support($subtype);
...
$icon = FALSE;
if ($image_support && ($image_support !== EventsBaseMain::$Events_st_OWNER_ICON_NAME)) {
//Get icon
...
?>
<div class='ktFormCont ktFormContEvent'>
	<?php // Title Section  ?>

	<?php // Picture and entity fields Section ?>
	<div class='ktFormContInner'>
		<?php if ($image_support && $icon) { ?>
			<div class="ktFormContIcon">
				<?php
					echo $icon;
					if ($vars['entity']->isCanceled()) {
						$event_canceled = elgg_view('events/event_canceled', array('entity' => $vars['entity']));
						echo $event_canceled;
					}
				?>
			</div>
		...
		<?php if (is_array($object_fields) && count($object_fields) > 0) { ?>
			<ul class='ktFormFields ktFormProfile'>
				...
				foreach ($object_fields as $internalname => $field_data) {
					$tmp_classname = EventsBaseMain::ktform_camelize_string($internalname, FALSE);
	...
	$profile_btns = elgg_view('events/profile/buttons', $vars);
	...
	<div class='ktFormDescCont'>
		<?php foreach ($object_description_fields as $desc_internalname => $description_field) { ?>
			<div class="ktFormDescInner <?php echo EventsBaseMain::ktform_camelize_string($desc_internalname, FALSE) ?>">
	...
	<?php
	echo elgg_view('events/profile/profile_footer', $vars);
	?>
</div>