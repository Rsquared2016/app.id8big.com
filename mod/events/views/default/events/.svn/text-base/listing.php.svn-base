...
$image_support = EventsBaseMain::ktform_get_entity_image_support($entity_subtype);
...
//$kt_chck = defined('Events_st_DISABLE_BULK_ACTIONS') && Events_st_DISABLE_BULK_ACTIONS != TRUE; // yes, there is one!
...
$subtitle = $subtitle.elgg_view('events/listing/subtitle_extended', $vars);
...
$tags = $tags.elgg_view('events/listing/tags_extended', $vars);
...
if ($entity->isCanceled()) {
	$event_canceled = elgg_view('events/event_canceled', array('entity' => $entity));
	$icon .= $event_canceled;
}
