...
/**
 * EventsBaseForm profile display
 *
...
if ($vars['full_view']) {
	...
	switch($display_type) {
		case 'full':
				echo elgg_view('events/profile_full', $vars);
			break;
		default:
				echo elgg_view('events/profile', $vars);
			break;
	}
	...
	switch ($option_viewtype) {
		...
		case 'full':
			echo elgg_view('events/listing_full', $vars);
		break;

		case 'gallery':
			echo elgg_view('events/gallery', $vars);
		break;

		case 'widget':
			echo elgg_view('events/listing_widget', $vars);
		break;

		default:
			echo elgg_view('events/listing', $vars);
		break;
	}
}
