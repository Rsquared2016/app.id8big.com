...
function blog_get_page_content_read($guid = NULL) {
	...
	if ($blog->comments_on != 'Off') {
        ...
        $return['content'] .= elgg_view_comments($blog, $add_comment);
	}
...
function blog_get_page_content_list($container_guid = NULL) {
	...
	if ($container_guid) {
		...
		if ($container instanceof ProjectGroup) {
			$return['title'] = elgg_echo('blog_to_news:title:user_blogs');
		}
		else {
	...
	if (!$list) {
		$return['content'] = '<p class="blogsNone">'.elgg_echo('blog:none').'</p>';
	} else {
		$return['content'] = $list;
	}
