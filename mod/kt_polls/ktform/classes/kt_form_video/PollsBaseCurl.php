...
/*
 * This is a class from izap videos, with some modifications to be used into kt_polls_ktform.
 * Thank you Izap Guys. :)
 */
  ...
  function &asPostString(&$theData, $theName = NULL) {
    ...
    if (is_array($theData)) {
      foreach ($theData as $theKey => $theValue) {
        if ($thePrefix === NULL) {
          $thePostString .= '&' . PollsBaseCurl::asPostString($theValue, $theKey) ;
        }
        else {
          $thePostString .= '&' . PollsBaseCurl::asPostString($theValue, $thePrefix . '[' . $theKey . ']') ;
        }
      }
    }
    else {
...
class kt_polls_xml2array {
  function kt_polls_xml2array( $xml ) {
    if ( file_exists($xml) )
