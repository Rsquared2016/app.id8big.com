...
if (isset($vars['extra_class']) && !empty($vars['extra_class'])) {
	...
	if (!empty($entity_subtype)) {
		...
		$extra_class = PollsBaseMain::ktform_camelize_string($entity_subtype, false);
...
$error_msg = elgg_echo("kt_polls_ktform:entities:{$entity_subtype}:listing:empty_results");
