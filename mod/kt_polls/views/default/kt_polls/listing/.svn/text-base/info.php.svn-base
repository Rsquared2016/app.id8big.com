...
//kt_polls_ktform_get_trimed_title().
$title = $entity->title;
if (!$title) {
	$title = $entity->name;
}
...
if(strlen($title) > Polls_st_MAX_LENGHT_TITLE_LISTING){
	$title = elgg_get_excerpt($title, Polls_st_MAX_LENGHT_TITLE_LISTING);
}
?>
...
?>
	<h4>
		...
		<?php
			...
			echo elgg_view('kt_polls/listing/info_middle_extend', $vars);
...
<?php
	 ...
	 * */
	$extra_fields = PollsBaseMain::ktform_get_extra_listing_fields($entity_subtype);
 ...
 * */
//echo elgg_view('kt_polls/tools/action_wheel', $vars);
