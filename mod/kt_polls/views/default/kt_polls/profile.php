...
$image_support = PollsBaseMain::ktform_get_entity_image_support($subtype);
...
$icon = FALSE;
if ($image_support && ($image_support !== PollsBaseMain::$Polls_st_OWNER_ICON_NAME)) {
//Get icon
...
<div class='ktFormCont'>
		...
		<?php if (is_array($object_fields) && count($object_fields) > 0) { ?>
			<ul class='ktFormFields ktFormProfile'>
				...
				foreach ($object_fields as $internalname => $field_data) {
					$tmp_classname = PollsBaseMain::ktform_camelize_string($internalname, FALSE);
    ...
    set_input('entity_owner_filter', true);
	
	if($entity->canSubmitPollQuiz(elgg_get_logged_in_user_entity())) {
		$form_element = '';
		$question_type = $entity->getQuestionType();
		
		switch ($question_type) {
			case 'poll':
				$form_element = $entity->generatePollElements();
				break;

			case 'quiz':
				$form_element = $entity->generateQuizElements();
				break;
		}

		$vars['form_element'] = $form_element;
		
		$profile_btns = elgg_view('kt_polls/poll/vote', $vars);
	} else {
		$profile_btns = elgg_view('kt_polls/poll/result', $vars);
	}
...
//	$profile_btns = elgg_view('kt_polls/poll/result', $vars);
	...
	<div class='ktFormDescCont'>
		<?php foreach ($object_description_fields as $desc_internalname => $description_field) { ?>
			<div class="ktFormDescInner <?php echo PollsBaseMain::ktform_camelize_string($desc_internalname, FALSE) ?>">
	...
	<?php
	echo elgg_view('kt_polls/profile/profile_footer', $vars);
	?>
</div>