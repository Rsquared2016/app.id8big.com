 ...
 *
 * @warning Only a single input/userprojectpicker is supported per web page.
 *
...
elgg_load_js('elgg.userprojectpicker');
...
function user_picker_add_user($user_id) {
	...
	// this html must be synced with the userprojectpicker.js library
	$code = '<li><div class="elgg-image-block">';
	$code .= "<div class='elgg-image'>$icon</div>";
	$code .= "<div class='elgg-image-alt'><a href='#' class='elgg-userprojectpicker-remove'>X</a></div>";
	$code .= "<div class='elgg-body'>" . $user->name . "</div>";
	$code .= "</div>";
	$code .= "<input type=\"hidden\" name=\"user_guid[]\" value=\"$user_id\">";
	$code .= '</li>';
...
<div class="elgg-user-picker">
	<input type="text" class="elgg-input-user-project-picker" size="30" id="userprojectpicker-element" data-entity-guid="<?php echo $entity->getGUID(); ?>"/>
...
<script type="text/javascript">
	...
	elgg.userprojectpicker.userList = <?php echo $json_values ?>;
</script>