...
if (!$title || !$desc) {
	register_error(elgg_echo('project_discussion:error:missing'));
	forward(REFERER);
}
...
if (!$result) {
	register_error(elgg_echo('project_discussion:error:notsaved'));
	forward(REFERER);
}
...
if ($new_topic) {
	system_message(elgg_echo('project_discussion:topic:created'));
	add_to_river('river/object/projectforumtopic/create', 'create', elgg_get_logged_in_user_guid(), $topic->guid);
} else {
	system_message(elgg_echo('project_discussion:topic:updated'));
}
